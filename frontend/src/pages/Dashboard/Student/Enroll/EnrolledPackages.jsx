    ...
    return (
        <div>
            ...
            <div className="grid md:grid-cols-2 gap-8">
                {
                    paginatedData.map(item => <div key={item.Packages._id + v4()} className="
                bg-white
                shadow-md
                h-96
                mx-3
              ...
              ">
                ...
                ">
                            <div className="flex flex-col justify-start items-baseline">
                                <h1 title={item.Packages.name} className="text-lg font-normal mb-0 text-gray-600 font-sans">
                                    {item.Packages.name.length > 20 ? item.Packages.name.slice(0, 20) + '...' : item.Packages.name}
                                </h1>
                                <span className="text-xs text-indigo-300 mt-0">by <span className='text-black'>{item.Packages.instructorName}</span></span>
                            </div>
                            <p className="text-xs text-gray-500 w-4/5">
                                {item.Packages.description?.length > 100 ? item.Packages.description.slice(0, 100) + '...' : item.Packages.description}
                            </p>
                            <div className="w-full flex justify-between items-center">
                                <h1 className="font-bold text-gray-500">${item.Packages.price}</h1>
                                <button
